...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                nextState = ((nextx, nexty), list(visitedCorners))  # Avoiding in-place modification
...
def cornersHeuristic(state, problem):
    ...
    visitedCorners = state[1]
    cornersLeftToVisit = [corner for corner in corners if corner not in visitedCorners]

    # While not all corners are visited, find the closest corner using manhattanDistance
    totalCost = 0
    curPoint = state[0]
    while cornersLeftToVisit:
        heuristic_cost, corner = min([(util.manhattanDistance(curPoint, corner), corner) for corner in
                                      cornersLeftToVisit])
...
def foodHeuristic(state, problem):
    ...
    If using A* ever finds a solution that is worse than a previous solution found,
    your heuristic is not consistent, and probably not admissible! On the other hand,
    inadmissible or inconsistent heuristics may find optimal solutions, but can never
    guarantee them.
    
    The state is a tuple ( pacmanPosition, foodGrid ) where foodGrid is a Grid (see
